...
const messagesToBody = messages => jsonToString(messages);
...
class AtatusLogger {
    constructor({
        ...
        if (!apiKey) {
            throw new Error('You are required to supply an api key for logging.');
        }

        this.apiKey = apiKey;
        this.host = host || 'log-rx.atatus.com';
        this.type = type;
        ...
        this.url = `${this.protocol}://${this.host}/track/logs/ingest`;
        ...
        this.axiosInstance.defaults.headers.post = {
            ...
            'user-agent': `NodeJS/${version} logs`,
            ...(this.compress ? { 'content-encoding': 'gzip' } : {}),

        };

        /*
          Callback method executed on each bulk of messages sent to atatus.
    ...
    _defaultCallback(err) {
        if (err && !this.supressErrors) {
            this.internalLogger.log(`atatus-logger error: ${err}`, err);
        }
    }
    ...
    _debug(msg) {
        if (this.debug) this.internalLogger.log(`atatus-logger: ${msg}`);
    }
...
const createLogger = (options) => {
    const l = new AtatusLogger(options);
    l._timerSend();
    return l;
};
